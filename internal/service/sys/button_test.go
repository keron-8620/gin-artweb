...
// CreateTestButtonDTO 创建测试用的按钮DTO
...
func (suite *ButtonTestSuite) SetupSuite() {
	...
	suite.buttonservice = NewButtonService(
		logger,
		syssvc.NewApiRepo(
			logger,
			db,
...
func (suite *ButtonTestSuite) TestGetMenu() {
	// 测试获取不存在的菜单
	menuID := uint32(9999)
	_, err := suite.buttonservice.GetMenu(context.Background(), menuID)
...
func (suite *ButtonTestSuite) TestGetMenu_ContextError() {
	...
	// 尝试使用已取消的上下文获取菜单
	menuID := uint32(1)
	_, err := suite.buttonservice.GetMenu(ctx, menuID)
...
func (suite *ButtonTestSuite) TestGetApis() {
	// 测试获取空API列表
	emptyApiIDs := []uint32{}
	apis, err := suite.buttonservice.GetApis(context.Background(), emptyApiIDs)
	...
	apis, err = suite.buttonservice.GetApis(context.Background(), nonExistentApiIDs)
...
func (suite *ButtonTestSuite) TestGetApis_ContextError() {
	...
	// 尝试使用已取消的上下文获取API列表
	apiIDs := []uint32{1}
	_, err := suite.buttonservice.GetApis(ctx, apiIDs)
...
func (suite *ButtonTestSuite) TestCreateButton() {
	...
	err := suite.buttonservice.menuRepo.CreateModel(context.Background(), testMenu, nil)
	suite.Nil(err, "创建菜单应该成功")
	suite.Greater(testMenu.ID, uint32(0), "菜单ID应该大于0")

	// 创建按钮
	testButton := CreateTestButtonDTO(testMenu.ID, []uint32{})
	createdButton, err := suite.buttonservice.CreateButton(context.Background(), testButton)
...
func (suite *ButtonTestSuite) TestCreateButton_ContextError() {
	...
	// 尝试使用已取消的上下文创建按钮
	testButton := CreateTestButtonDTO(1, []uint32{})
	_, err := suite.buttonservice.CreateButton(ctx, testButton)
...
func (suite *ButtonTestSuite) TestFindButtonByID() {
	...
	// 测试查找刚创建的按钮
	foundButton, err := suite.buttonservice.FindButtonByID(context.Background(), []string{}, createdButton.ID)
...
func (suite *ButtonTestSuite) TestFindButtonByID_ContextError() {
	...
	// 尝试使用已取消的上下文查找按钮
	_, err := suite.buttonservice.FindButtonByID(ctx, []string{}, 1)
...
func (suite *ButtonTestSuite) TestUpdateButtonByID() {
	...
	updateDTO := sysmodel.UpdateButtonDTO{
		Name:     "Updated Button",
		Sort:     createdButton.Sort,
		IsActive: false,
		Descr:    "这是一个更新后的测试按钮",
		MenuID:   createdButton.MenuID,
		ApiIDs:   []uint32{},
	}

	// 执行更新
	updatedButton, err := suite.buttonservice.UpdateButtonByID(context.Background(), createdButton.ID, updateDTO)
	suite.Nil(err, "更新按钮应该成功")
	suite.Equal(createdButton.ID, updatedButton.ID)
	suite.Equal(updateDTO.Name, updatedButton.Name)
	suite.Equal(updateDTO.Descr, updatedButton.Descr)
	suite.Equal(updateDTO.IsActive, updatedButton.IsActive)
}
...
func (suite *ButtonTestSuite) TestUpdateButtonByID_ContextError() {
	...
	_, err := suite.buttonservice.UpdateButtonByID(ctx, 1, updateDTO)
...
func (suite *ButtonTestSuite) TestDeleteButtonByID() {
	...
	// 测试删除刚创建的按钮
	err = suite.buttonservice.DeleteButtonByID(context.Background(), createdButton.ID)
	suite.Nil(err, "删除刚创建的按钮应该成功")

	// 验证按钮已被删除
	_, err = suite.buttonservice.FindButtonByID(context.Background(), []string{}, createdButton.ID)
...
func (suite *ButtonTestSuite) TestDeleteButtonByID_ContextError() {
	...
	// 尝试使用已取消的上下文删除按钮
	err := suite.buttonservice.DeleteButtonByID(ctx, 1)
...
func (suite *ButtonTestSuite) TestListButton() {
	...
	for i := 0; i < buttonCount; i++ {
		testButton := CreateTestButtonDTO(testMenu.ID, []uint32{})
		_, err := suite.buttonservice.CreateButton(context.Background(), testButton)
		suite.Nil(err, "创建按钮应该成功")
	}

	// 测试列出所有按钮
	listDTO := sysmodel.ListButtonDTO{}
	count, buttonList, err := suite.buttonservice.ListButton(context.Background(), 1, 10, listDTO)
...
func (suite *ButtonTestSuite) TestListButton_ContextError() {
	...
	// 尝试使用已取消的上下文列出按钮
	listDTO := sysmodel.ListButtonDTO{}
	_, _, err := suite.buttonservice.ListButton(ctx, 1, 10, listDTO)
...
func (suite *ButtonTestSuite) TestLoadButtonPolicy() {
	...
	// 加载按钮策略
	err = suite.buttonservice.LoadButtonPolicy(context.Background())
...
func (suite *ButtonTestSuite) TestLoadButtonPolicy_ContextError() {
	...
	// 尝试使用已取消的上下文加载按钮策略
	err := suite.buttonservice.LoadButtonPolicy(ctx)
...
func (suite *ButtonTestSuite) TestCreateButtonWithApis() {
	...
	for i := 0; i < apiCount; i++ {
		...
		err := suite.buttonservice.apiRepo.CreateModel(context.Background(), testApi)
	...
	// 创建关联API的按钮
	testButton := CreateTestButtonDTO(testMenu.ID, apiIDs)
	createdButton, err := suite.buttonservice.CreateButton(context.Background(), testButton)
	...
	// 验证按钮关联的API
	foundButton, err := suite.buttonservice.FindButtonByID(context.Background(), []string{"Apis"}, createdButton.ID)

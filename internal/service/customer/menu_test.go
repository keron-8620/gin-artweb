...
func CreateTestMenuModel(parentID *uint32) *custmodel.MenuModel {
	return &custmodel.MenuModel{
		ParentID:  parentID,
		Name:      uuid.NewString(),
		Path:      uuid.NewString(),
		Component: "TestMenu",
...
func (suite *MenuTestSuite) TestCreateMenu() {
	...
	// 创建菜单
	createdMenu, err := suite.menuservice.CreateMenu(context.Background(), apiIDs, *testMenu)
...
func (suite *MenuTestSuite) TestCreateMenu_ContextError() {
	...
	testMenu := CreateTestMenuModel(nil)
	apiIDs := []uint32{}
	_, err := suite.menuservice.CreateMenu(ctx, apiIDs, *testMenu)
...
func (suite *MenuTestSuite) TestUpdateMenuByID() {
	...
	// 执行更新
	updatedMenu, err := suite.menuservice.UpdateMenuByID(context.Background(), createdMenu.ID, apiIDs, updateData)
...
func (suite *MenuTestSuite) TestUpdateMenuByID_ContextError() {
	...
	apiIDs := []uint32{}
	_, err := suite.menuservice.UpdateMenuByID(ctx, 1, apiIDs, updateData)
...
func (suite *MenuTestSuite) TestListMenu() {
	...
	for i := 0; i < menuCount; i++ {
		testMenu := CreateTestMenuModel(nil)
		apiIDs := []uint32{}
		_, err := suite.menuservice.CreateMenu(context.Background(), apiIDs, *testMenu)
		suite.Nil(err, "创建菜单应该成功")
	}

	// 测试列出所有菜单
	qp := database.QueryParams{}
	count, menuList, err := suite.menuservice.ListMenu(context.Background(), qp)
...
func (suite *MenuTestSuite) TestListMenu_ContextError() {
	...
	// 尝试使用已取消的上下文列出菜单
	qp := database.QueryParams{}
	_, _, err := suite.menuservice.ListMenu(ctx, qp)

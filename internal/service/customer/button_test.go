...
func CreateTestButtonModel(menuID uint32) *custmodel.ButtonModel {
	return &custmodel.ButtonModel{
		MenuID:   menuID,
		Name:     uuid.NewString(),
		Sort:     10000,
		IsActive: true,
...
func (suite *ButtonTestSuite) SetupSuite() {
	...
	db.AutoMigrate(
		&custmodel.MenuModel{},
		&custmodel.ApiModel{},
		&custmodel.ButtonModel{},
	)
	dbTimeout := test.NewTestDBTimeouts()
	logger := test.NewTestZapLogger()
	enforcer, _ := auth.NewCasbinEnforcer()
	suite.enforcer = enforcer
	suite.buttonservice = &ButtonService{
		log: logger,
		apiRepo: custsvc.NewApiRepo(
			logger,
			db,
			dbTimeout,
			enforcer,
		),
		menuRepo: custsvc.NewMenuRepo(
			logger,
			db,
			dbTimeout,
			enforcer,
		),
		buttonRepo: custsvc.NewButtonRepo(
			logger,
			db,
			dbTimeout,
			enforcer,
...
func (suite *ButtonTestSuite) TestCreateButton() {
	...
	apiIDs := []uint32{}
	createdButton, err := suite.buttonservice.CreateButton(context.Background(), apiIDs, *testButton)
...
func (suite *ButtonTestSuite) TestCreateButton_ContextError() {
	...
	apiIDs := []uint32{}
	_, err := suite.buttonservice.CreateButton(ctx, apiIDs, *testButton)
...
func (suite *ButtonTestSuite) TestUpdateButtonByID() {
	...
	// 执行更新
	updatedButton, err := suite.buttonservice.UpdateButtonByID(context.Background(), createdButton.ID, apiIDs, updateData)
...
func (suite *ButtonTestSuite) TestUpdateButtonByID_ContextError() {
	...
	apiIDs := []uint32{}
	_, err := suite.buttonservice.UpdateButtonByID(ctx, 1, apiIDs, updateData)
...
func (suite *ButtonTestSuite) TestListButton() {
	...
	for i := 0; i < buttonCount; i++ {
		testButton := CreateTestButtonModel(testMenu.ID)
		apiIDs := []uint32{}
		_, err := suite.buttonservice.CreateButton(context.Background(), apiIDs, *testButton)
		suite.Nil(err, "创建按钮应该成功")
	}

	// 测试列出所有按钮
	qp := database.QueryParams{}
	count, buttonList, err := suite.buttonservice.ListButton(context.Background(), qp)
...
func (suite *ButtonTestSuite) TestListButton_ContextError() {
	...
	// 尝试使用已取消的上下文列出按钮
	qp := database.QueryParams{}
	_, _, err := suite.buttonservice.ListButton(ctx, qp)

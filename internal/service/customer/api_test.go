...
func CreateTestApiModel() *custmodel.ApiModel {
	return &custmodel.ApiModel{
...
func (suite *ApiTestSuite) SetupSuite() {
	...
	db.AutoMigrate(&custmodel.ApiModel{})
...
func (suite *ApiTestSuite) TestUpdateApiByID() {
	...
	// 执行更新
	updatedFm, err := suite.apiservice.UpdateApiByID(context.Background(), fm.ID, updateData)
...
func (suite *ApiTestSuite) TestUpdateApiByID_NotFound() {
	...
	_, err := suite.apiservice.UpdateApiByID(context.Background(), 0, updateData)
...
func (suite *ApiTestSuite) TestListApi() {
	...
	for i := 0; i < apiCount; i++ {
		sm := CreateTestApiModel()
		_, err := suite.apiservice.CreateApi(context.Background(), *sm)
		suite.Nil(err, "创建API应该成功")
	}

	// 测试列出所有API
	qp := database.QueryParams{}
	count, apiList, err := suite.apiservice.ListApi(context.Background(), qp)
...
func (suite *ApiTestSuite) TestCreateApi_ContextError() {
	...
	// 尝试使用已取消的上下文创建API
	sm := CreateTestApiModel()
	_, err := suite.apiservice.CreateApi(ctx, *sm)
...
func (suite *ApiTestSuite) TestUpdateApiByID_ContextError() {
	...
	_, err := suite.apiservice.UpdateApiByID(ctx, 1, updateData)
...
func (suite *ApiTestSuite) TestListApi_ContextError() {
	...
	// 尝试使用已取消的上下文列出API
	qp := database.QueryParams{}
	_, _, err := suite.apiservice.ListApi(ctx, qp)

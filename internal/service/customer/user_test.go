...
func CreateTestUserModel(roleID uint32) *custmodel.UserModel {
	return &custmodel.UserModel{
...
func (suite *UserTestSuite) SetupSuite() {
	...
	db.AutoMigrate(
		...
		&custmodel.RoleModel{},
		&custmodel.UserModel{},
		&custmodel.LoginRecordModel{},
	)
	dbTimeout := test.NewTestDBTimeouts()
	logger := test.NewTestZapLogger()
	enforcer, _ := auth.NewCasbinEnforcer()

	suite.uc = &UserService{
		log: logger,
		...
		userRepo: custsvc.NewUserRepo(
			logger,
			db,
			dbTimeout,
		),
		recordRepo: custsvc.NewLoginRecordRepo(
			logger,
			db,
			dbTimeout,

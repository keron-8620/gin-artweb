...
func CreateTestRoleModel() *custmodel.RoleModel {
	return &custmodel.RoleModel{
		Name:  uuid.NewString(),
		Descr: "这是一个测试角色",
	}
}
...
func (suite *RoleTestSuite) TestFindRoleByID() {
	...
	createdRole, err := suite.roleservice.CreateRole(context.Background(), []uint32{}, []uint32{}, []uint32{}, *testRole)
...
func (suite *RoleTestSuite) TestListRole() {
	...
	for i := 0; i < roleCount; i++ {
		testRole := CreateTestRoleModel()
		_, err := suite.roleservice.CreateRole(context.Background(), []uint32{}, []uint32{}, []uint32{}, *testRole)
		suite.Nil(err, "创建角色应该成功")
	}

	// 测试查询角色列表
	count, roles, err := suite.roleservice.ListRole(context.Background(), database.QueryParams{})
...
func (suite *RoleTestSuite) TestGetRoleMenuTree() {
	...
	// 创建测试角色并关联菜单
	testRole := CreateTestRoleModel()
	createdRole, err := suite.roleservice.CreateRole(context.Background(), []uint32{}, []uint32{testMenu.ID}, []uint32{}, *testRole)
	suite.Nil(err, "创建角色应该成功")
...
func (suite *RoleTestSuite) TestCreateRoleWithRelations() {
	...
	// 创建测试角色并关联API、菜单、按钮
	testRole := CreateTestRoleModel()
	createdRole, err := suite.roleservice.CreateRole(context.Background(), []uint32{testApi.ID}, []uint32{testMenu.ID}, []uint32{testButton.ID}, *testRole)
	suite.Nil(err, "创建角色应该成功")
	suite.NotNil(createdRole, "角色不应该为空")
...
func (suite *RoleTestSuite) TestUpdateRoleByID() {
	...
	updatedRole, err := suite.roleservice.UpdateRoleByID(context.Background(), createdRole.ID, []uint32{}, []uint32{}, []uint32{}, map[string]any{
		"name":  updatedName,
		"descr": updatedDescr,
	})
...
func (suite *RoleTestSuite) TestCreateRoleWithContextError() {
	...
	// 测试上下文错误
	testRole := CreateTestRoleModel()
	_, err := suite.roleservice.CreateRole(ctx, []uint32{}, []uint32{}, []uint32{}, *testRole)
	suite.NotNil(err, "上下文错误应该返回错误")
}
...
func (suite *RoleTestSuite) TestUpdateRoleByIDWithContextError() {
	...
	// 测试上下文错误
	_, err := suite.roleservice.UpdateRoleByID(ctx, 1, []uint32{}, []uint32{}, []uint32{}, map[string]any{})
	suite.NotNil(err, "上下文错误应该返回错误")
}
...
func (suite *RoleTestSuite) TestListRoleWithContextError() {
	...
	// 测试上下文错误
	_, _, err := suite.roleservice.ListRole(ctx, database.QueryParams{})
	suite.NotNil(err, "上下文错误应该返回错误")
}
...
func (suite *RoleTestSuite) TestRoleApiRelations() {
	...
	// 创建角色并关联API
	testRole := CreateTestRoleModel()
	createdRole, err := suite.roleservice.CreateRole(context.Background(), apiIDs, []uint32{}, []uint32{}, *testRole)
	suite.Nil(err, "创建角色应该成功")
...
func (suite *RoleTestSuite) TestRoleMenuRelations() {
	...
	// 创建角色并关联菜单
	testRole := CreateTestRoleModel()
	createdRole, err := suite.roleservice.CreateRole(context.Background(), []uint32{}, menuIDs, []uint32{}, *testRole)
	suite.Nil(err, "创建角色应该成功")
...
func (suite *RoleTestSuite) TestRoleButtonRelations() {
	...
	// 创建角色并关联按钮
	testRole := CreateTestRoleModel()
	createdRole, err := suite.roleservice.CreateRole(context.Background(), []uint32{}, []uint32{testMenu.ID}, buttonIDs, *testRole)
	suite.Nil(err, "创建角色应该成功")

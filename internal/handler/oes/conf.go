...
func (s *OesConfService) UploadOesConf(ctx *gin.Context) {
	// 1. 绑定URL路径参数
	var pathReq oesmodel.GetOesConfRequest
	...
	// 2. 绑定表单数据（包含文件）
	var formReq oesmodel.UploadOesConfRequest
...
func (s *OesConfService) DownloadOesConf(ctx *gin.Context) {
	var req oesmodel.DownloadOrDeleteOesConfRequest
...
// @Produce json
// @Param colony_num path string true "集群编号"
// @Success 200 {object} oesmodel.PagOesConfReply "成功返回配置文件列表"
// @Failure 400 {object} errors.Error "请求参数错误"
// @Failure 500 {object} errors.Error "服务器内部错误"
// @Router /api/v1/oes/{colony_num}/conf [get]
// @Security ApiKeyAuth
func (s *OesConfService) ListOesConf(ctx *gin.Context) {
	var req oesmodel.ListOesConfRequest
	if err := ctx.ShouldBindUri(&req); err != nil {
		s.log.Error(
			"绑定oes配置文件路径参数失败",
			zap.Error(err),
	...
	ctx.JSON(http.StatusOK, oesmodel.PagOesConfReply{
		Code: http.StatusOK,
		Data: info,
	})
}

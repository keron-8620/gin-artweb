...
//  2. 执行数据库创建操作
//  3. 记录操作日志
func (r *ScheduleRepo) CreateModel(ctx context.Context, m *model.ScheduleModel) error {
	// 检查参数
	if m == nil {
	...
	defer cancel()
	if err := database.DBCreate(dbCtx, r.gormDB, &model.ScheduleModel{}, m, nil); err != nil {
		r.log.Error(
			"创建计划任务模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *ScheduleRepo) UpdateModel(ctx context.Context, data map[string]any, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBUpdate(dbCtx, r.gormDB, &model.ScheduleModel{}, data, nil, conds...); err != nil {
		r.log.Error(
			"更新计划任务模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *ScheduleRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBDelete(dbCtx, r.gormDB, &model.ScheduleModel{}, conds...); err != nil {
		r.log.Error(
			"删除计划任务模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*model.ScheduleModel: 计划任务模型指针，包含计划任务的详细信息
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
//
//	int64: 查询结果总数
//	*[]model.ScheduleModel: 计划任务模型列表指针，包含查询到的计划任务详情
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *ScheduleRepo) ListModel(
...
) (int64, *[]model.ScheduleModel, error) {
	r.log.Debug(
		"开始查询计划任务模型列表",
	...
	startTime := time.Now()
	var ms []model.ScheduleModel
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.ListTimeout)
	defer cancel()
	count, err := database.DBList(dbCtx, r.gormDB, &model.ScheduleModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询计划任务模型列表失败",
			zap.Error(err),

...
//  3. 执行数据库创建操作
//  4. 记录操作日志
func (r *ScriptRepo) CreateModel(ctx context.Context, m *jobmodel.ScriptModel) error {
	...
	defer cancel()
	if err := database.DBCreate(dbCtx, r.gormDB, &jobmodel.ScriptModel{}, m, nil); err != nil {
		log.Error(
			"创建脚本模型：数据库操作失败",
			zap.Error(err),
			zap.Object("script_model", m),
...
func (r *ScriptRepo) UpdateModel(ctx context.Context, data map[string]any, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBUpdate(dbCtx, r.gormDB, &jobmodel.ScriptModel{}, data, nil, conds...); err != nil {
		log.Error(
			"更新脚本模型：数据库操作失败",
			zap.Error(err),
			zap.Any("update_data", data),
...
func (r *ScriptRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBDelete(dbCtx, r.gormDB, &jobmodel.ScriptModel{}, conds...); err != nil {
		log.Error(
			"删除脚本模型：数据库操作失败",
			zap.Error(err),
			zap.Any("conds", conds),
...
func (r *ScriptRepo) ListModel(
	...
	defer cancel()
	if err := database.DBList(dbCtx, r.gormDB, &jobmodel.ScriptModel{}, &ms, qp); err != nil {
		log.Error(
			"查询脚本模型列表：数据库操作失败",
			zap.Error(err),
			zap.Object("query_params", &qp),
...
func (r *ScriptRepo) SaveScriptFile(
	...
	if err := fileutil.WriteReaderToFile(ctx, fileReader, scriptPath, os.FileMode(0o750), overwrite); err != nil {
		log.Error(
			"保存脚本文件：文件写入失败",
			zap.Error(err),
			zap.String("script_path", scriptPath),
...
func (r *ScriptRepo) RemoveScriptFile(ctx context.Context, scriptPath string) error {
	log := ctxutil.NewLogger(r.log, ctx)
...
func (r *ScriptRepo) ListProjects(
	...
	var projects []string
	if err := r.gormDB.WithContext(dbCtx).Model(&jobmodel.ScriptModel{}).Where(query).Distinct("project").Pluck("project", &projects).Error; err != nil {
		log.Error(
			"查询脚本所有的项目名称：数据库查询失败",
			zap.Error(err),
			zap.Any("query_params", query),
...
func (r *ScriptRepo) ListLabels(
	...
	// 查询所有唯一的标签名称
	if err := r.gormDB.WithContext(dbCtx).Model(&jobmodel.ScriptModel{}).Where(query).Distinct("label").Pluck("label", &labels).Error; err != nil {
		log.Error(
			"查询脚本所有的标签名称：数据库查询失败",
			zap.Error(err),
			zap.Any("query_params", query),

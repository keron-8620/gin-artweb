...
func NewJobRouter(
	...
	scriptRepo := jobrepo.NewScriptRepo(loggers.Repo, init.DB, init.DBTimeout, init.DBSlowThreshold)
	recordRepo := jobrepo.NewRecordRepo(loggers.Repo, init.DB, init.DBTimeout, init.DBSlowThreshold)
	scheduleRepo := jobrepo.NewScheduleRepo(loggers.Repo, init.DB, init.DBTimeout, init.DBSlowThreshold)
	...
	// 加载计划任务
	if err := scheduleService.LoadSchedule(context.Background()); err != nil {
		panic(err)
	}

...
// 返回值：
//
//	jobsmodel.ScriptRecordRepo: 脚本执行记录仓库接口实现
func NewRecordRepo(
	log *zap.Logger,
	gormDB *gorm.DB,
...
//  2. 执行数据库创建操作
//  3. 记录操作日志
func (r *RecordRepo) CreateModel(ctx context.Context, m *jobsmodel.ScriptRecordModel) error {
	// 检查参数
	if m == nil {
	...
	defer cancel()
	if err := database.DBCreate(dbCtx, r.gormDB, &jobsmodel.ScriptRecordModel{}, m, nil); err != nil {
		r.log.Error(
			"创建脚本执行记录模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *RecordRepo) UpdateModel(ctx context.Context, data map[string]any, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBUpdate(dbCtx, r.gormDB, &jobsmodel.ScriptRecordModel{}, data, nil, conds...); err != nil {
		r.log.Error(
			"更新脚本执行记录模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *RecordRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBDelete(dbCtx, r.gormDB, &jobsmodel.ScriptRecordModel{}, conds...); err != nil {
		r.log.Error(
			"删除脚本执行记录模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*jobsmodel.ScriptRecordModel: 脚本执行记录模型指针，包含脚本执行记录的详细信息
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *RecordRepo) GetModel(
	...
	preloads []string,
	conds ...any,
) (*jobsmodel.ScriptRecordModel, error) {
	r.log.Debug(
		"开始查询脚本执行记录模型",
		zap.Any(database.ConditionsKey, conds),
		zap.String(ctxutil.TraceIDKey, ctxutil.GetTraceID(ctx)),
	...
	startTime := time.Now()
	var m jobsmodel.ScriptRecordModel
...
//
//	int64: 查询结果总数
//	*[]jobsmodel.ScriptRecordModel: 脚本执行记录模型列表指针，包含查询到的脚本执行记录详情
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *RecordRepo) ListModel(
...
) (int64, *[]jobsmodel.ScriptRecordModel, error) {
	r.log.Debug(
		"开始查询脚本执行记录模型列表",
	...
	startTime := time.Now()
	var ms []jobsmodel.ScriptRecordModel
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.ListTimeout)
	defer cancel()
	count, err := database.DBList(dbCtx, r.gormDB, &jobsmodel.ScriptRecordModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询脚本执行记录模型列表失败",
			zap.Error(err),

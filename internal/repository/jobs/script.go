...
//  3. 执行数据库创建操作
//  4. 记录操作日志
func (r *ScriptRepo) CreateModel(ctx context.Context, m *jobsmodel.ScriptModel) error {
	// 检查参数
	if m == nil {
	...
	defer cancel()
	if err := database.DBCreate(dbCtx, r.gormDB, &jobsmodel.ScriptModel{}, m, nil); err != nil {
		r.log.Error(
			"创建脚本模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *ScriptRepo) UpdateModel(ctx context.Context, data map[string]any, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBUpdate(dbCtx, r.gormDB, &jobsmodel.ScriptModel{}, data, nil, conds...); err != nil {
		r.log.Error(
			"更新脚本模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *ScriptRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBDelete(dbCtx, r.gormDB, &jobsmodel.ScriptModel{}, conds...); err != nil {
		r.log.Error(
			"删除脚本模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*jobsmodel.ScriptModel: 脚本模型指针，包含脚本的详细信息
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
//
//	int64: 总记录数
//	*[]jobsmodel.ScriptModel: 脚本模型列表指针，包含符合条件的脚本模型
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *ScriptRepo) ListModel(
...
) (int64, *[]jobsmodel.ScriptModel, error) {
	r.log.Debug(
		"开始查询脚本模型列表",
	...
	startTime := time.Now()
	var ms []jobsmodel.ScriptModel
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.ListTimeout)
	defer cancel()
	count, err := database.DBList(dbCtx, r.gormDB, &jobsmodel.ScriptModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询脚本模型列表失败",
			zap.Error(err),
...
func (r *ScriptRepo) ListProjects(
	...
	var projects []string
	if err := r.gormDB.WithContext(dbCtx).Model(&jobsmodel.ScriptModel{}).Where(query).Distinct("project").Pluck("project", &projects).Error; err != nil {
		r.log.Error(
			"查询项目名称失败",
			zap.Error(err),
			zap.Any(database.QueryParamsKey, query),
...
func (r *ScriptRepo) ListLabels(
	...
	// 查询所有唯一的标签名称
	if err := r.gormDB.WithContext(dbCtx).Model(&jobsmodel.ScriptModel{}).Where(query).Distinct("label").Pluck("label", &labels).Error; err != nil {
		r.log.Error(
			"查询标签名称失败",
			zap.Error(err),
			zap.Any(database.QueryParamsKey, query),

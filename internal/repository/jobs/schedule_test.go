...
func CreateTestScheduleModel(scriptID uint32) *jobsmodel.ScheduleModel {
	return &jobsmodel.ScheduleModel{
		Name:          uuid.NewString(),
...
func (suite *ScheduleTestSuite) SetupSuite() {
	...
	db.AutoMigrate(
		&jobsmodel.ScriptModel{},
		&jobsmodel.ScheduleModel{},
	)
...
func (suite *ScheduleTestSuite) TestCreateModel() {
	...
	// 测试边界情况: 最小有效数据
	minimalSchedule := &jobsmodel.ScheduleModel{
		Name:          "minimal",
		Specification: "* * * * *",
		IsEnabled:     false,
		EnvVars:       "{}",

...
func (r *RoleRepo) CreateModel(
	...
	if err := database.DBCreate(ctx, r.gormDB, &custmodel.RoleModel{}, m, upmap); err != nil {
		r.log.Error(
			"创建角色模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *RoleRepo) UpdateModel(
	...
	if menus != nil {
		if len(*menus) > 0 {
			...
			upmap["Menus"] = []custmodel.MenuModel{}
		}
	}
	if buttons != nil {
		if len(*buttons) > 0 {
			...
			upmap["Buttons"] = []custmodel.ButtonModel{}
		}
	}
	if err := database.DBUpdate(ctx, r.gormDB, &custmodel.RoleModel{}, data, upmap, conds...); err != nil {
		r.log.Error(
			"更新角色模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *RoleRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	now := time.Now()
	if err := database.DBDelete(ctx, r.gormDB, &custmodel.RoleModel{}, conds...); err != nil {
		r.log.Error(
			"删除角色模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*custmodel.RoleModel: 角色模型指针，包含角色的详细信息
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
//
//	int64: 总记录数
//	*[]custmodel.RoleModel: 角色模型列表指针，包含符合条件的角色模型
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *RoleRepo) ListModel(
...
) (int64, *[]custmodel.RoleModel, error) {
	r.log.Debug(
		"开始查询角色模型列表",
	...
	now := time.Now()
	var ms []custmodel.RoleModel
	count, err := database.DBList(ctx, r.gormDB, &custmodel.RoleModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询角色模型列表失败",
			zap.Error(err),

...
func (r *MenuRepo) CreateModel(
	...
	defer cancel()
	if err := database.DBCreate(dbCtx, r.gormDB, &custmodel.MenuModel{}, m, upmap); err != nil {
		r.log.Error(
			"创建菜单模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *MenuRepo) UpdateModel(
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBUpdate(dbCtx, r.gormDB, &custmodel.MenuModel{}, data, upmap, conds...); err != nil {
		r.log.Error(
			"更新菜单模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *MenuRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.WriteTimeout)
	defer cancel()
	if err := database.DBDelete(dbCtx, r.gormDB, &custmodel.MenuModel{}, conds...); err != nil {
		r.log.Error(
			"删除菜单模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*custmodel.MenuModel: 查询到的菜单模型指针
//	error: 操作过程中的错误
func (r *MenuRepo) GetModel(
	ctx context.Context,
	preloads []string,
	conds ...any,
) (*custmodel.MenuModel, error) {
	r.log.Debug(
		"开始获取菜单模型",
...
//
//	int64: 总记录数
//	*[]custmodel.MenuModel: 菜单模型列表指针
//	error: 操作过程中的错误
func (r *MenuRepo) ListModel(
	ctx context.Context,
	qp database.QueryParams,
) (int64, *[]custmodel.MenuModel, error) {
	r.log.Debug(
		"开始查询菜单模型列表",
	...
	now := time.Now()
	var ms []custmodel.MenuModel
	dbCtx, cancel := context.WithTimeout(ctx, r.timeouts.ListTimeout)
	defer cancel()
	count, err := database.DBList(dbCtx, r.gormDB, &custmodel.MenuModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询菜单模型列表失败",
			zap.Error(err),
...
func ListMenuModelToUint32s(mms *[]custmodel.MenuModel) []uint32 {
	if mms == nil {
		return []uint32{}
	}
	ms := *mms

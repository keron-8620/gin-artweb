...
func (suite *MenuTestSuite) SetupSuite() {
	...
	db.AutoMigrate(&custmodel.MenuModel{}, &custmodel.ApiModel{})
...
func (suite *MenuTestSuite) TestCreateMenuWithParent() {
	apis := make([]custmodel.ApiModel, 3)
	for range 3 {
...
func (suite *MenuTestSuite) TestCreateMenuWithSortBoundaries() {
	// 测试 Sort=0 的情况
	smZeroSort := &custmodel.MenuModel{
		ParentID:  nil,
		Name:      uuid.NewString(),
		Path:      uuid.NewString(),
		Component: "TestMenu",
	...
	// 测试较大的 Sort 值
	smLargeSort := &custmodel.MenuModel{
		ParentID:  nil,
		Name:      uuid.NewString(),
		Path:      uuid.NewString(),
		Component: "TestMenu",
...
func (suite *MenuTestSuite) TestPreloadApis() {
	// 创建API
	apis := make([]custmodel.ApiModel, 2)
	for i := range 2 {
		m := CreateTestApiModel()
		suite.NoError(suite.apiRepo.CreateModel(context.Background(), m))
		apis[i] = *m

...
func (r *ButtonRepo) CreateModel(
	...
	if err := database.DBCreate(ctx, r.gormDB, &custmodel.ButtonModel{}, m, upmap); err != nil {
		r.log.Error(
			"创建按钮模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *ButtonRepo) UpdateModel(
	...
	if apis != nil {
		if len(*apis) > 0 {
			...
			upmap["Apis"] = []custmodel.ApiModel{}
		}
	}

	if err := database.DBUpdate(ctx, r.gormDB, &custmodel.ButtonModel{}, data, upmap, conds...); err != nil {
		r.log.Error(
			"更新按钮模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *ButtonRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	now := time.Now()
	if err := database.DBDelete(ctx, r.gormDB, &custmodel.ButtonModel{}, conds...); err != nil {
		r.log.Error(
			"删除按钮模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*custmodel.ButtonModel: 按钮模型指针，包含按钮的详细信息
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
//
//	int64: 总记录数
//	*[]custmodel.ButtonModel: 按钮模型列表指针，包含符合条件的按钮模型
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *ButtonRepo) ListModel(
...
) (int64, *[]custmodel.ButtonModel, error) {
	r.log.Debug(
		"开始查询按钮模型列表",
	...
	now := time.Now()
	var ms []custmodel.ButtonModel
	count, err := database.DBList(ctx, r.gormDB, &custmodel.ButtonModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询按钮模型列表失败",
			zap.Error(err),
...
func ListButtonModelToUint32s(bms *[]custmodel.ButtonModel) []uint32 {
	if bms == nil {
		return []uint32{}
	}
	ms := *bms

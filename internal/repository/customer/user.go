...
//  3. 执行数据库创建操作
//  4. 记录操作日志
func (r *UserRepo) CreateModel(ctx context.Context, m *custmodel.UserModel) error {
	// 检查参数
	if m == nil {
	...
	m.CreatedAt = now
	m.UpdatedAt = now
	if err := database.DBCreate(ctx, r.gormDB, &custmodel.UserModel{}, m, nil); err != nil {
		r.log.Error(
			"创建用户模型失败",
			zap.Error(err),
			zap.Object(database.ModelKey, m),
...
func (r *UserRepo) UpdateModel(ctx context.Context, data map[string]any, conds ...any) error {
	...
	now := time.Now()
	if err := database.DBUpdate(ctx, r.gormDB, &custmodel.UserModel{}, data, nil, conds...); err != nil {
		r.log.Error(
			"更新用户模型失败",
			zap.Error(err),
			zap.Any(database.UpdateDataKey, data),
...
func (r *UserRepo) DeleteModel(ctx context.Context, conds ...any) error {
	...
	now := time.Now()
	if err := database.DBDelete(ctx, r.gormDB, &custmodel.UserModel{}, conds...); err != nil {
		r.log.Error(
			"删除用户模型失败",
			zap.Error(err),
			zap.Any(database.ConditionsKey, conds),
...
// 返回值：
//
//	*custmodel.UserModel: 用户模型指针，包含用户的详细信息
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
//
//	int64: 总记录数
//	*[]custmodel.UserModel: 用户模型列表指针，包含符合条件的用户模型
//	error: 操作错误信息，成功则返回nil
//
// 功能：
//  1. 执行数据库查询操作
...
func (r *UserRepo) ListModel(
...
) (int64, *[]custmodel.UserModel, error) {
	r.log.Debug(
		"开始查询用户模型列表",
	...
	now := time.Now()
	var ms []custmodel.UserModel
	count, err := database.DBList(ctx, r.gormDB, &custmodel.UserModel{}, &ms, qp)
	if err != nil {
		r.log.Error(
			"查询用户列表失败",
			zap.Error(err),

...
if [ ! -f $bse_success_flag ];then
	...
	cd $dirpath/cmd/;sh bse.sh $colony_num "$@"
fi
# sse的success标志文件未生成
...
# 检测所有success标志文件都生成
if [ -f $mon_success_flag ]&&[ -f $counter_fetch_success_flag ]&&[ -f $counter_distribute_success_flag ]&&[ -f $bse_success_flag ]&&[ -f $sse_success_flag ]&&[ -f $szse_success_flag ]&&[ -f $csdc_success_flag ];then
...
# 检测所有ok标志文件都生成
if [ -f $fetch_OK_flag ]&&[ -f $freezed_OK_flag ];then
	echo "所有标志文件都已经生成, 开始重启oes集群服务器"
	sleep 5
	echo "sh oes_restart.sh $colony_num;sh oes_set_status.sh $colony_num RESET 5"

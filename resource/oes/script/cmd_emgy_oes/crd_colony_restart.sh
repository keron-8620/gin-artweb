...
flag_path=$(cd $dirpath/../../../../storage/oes/flags/$colony_num; pwd)
...
sse_late_success_flag=$flag_path/sse_late_collector_$nowdate.success
szse_late_success_flag=$flag_path/szse_late_collector_$nowdate.success
...
if [ ! -f $mon_success_flag ];then
	...
	cd $dirpath/../cmd/;sh mon.sh $colony_num "$@"
fi
...
if [ ! -f $counter_fetch_success_flag ];then
	...
	cd $dirpath/../cmd/;sh counter_fetch.sh $colony_num "$@"
fi
...
if [ -f $counter_fetch_success_flag ]&&[ ! -f $counter_distribute_success_flag ];then
	...
	cd $dirpath/../cmd/;sh counter_distribute.sh $colony_num "$@"
fi
# sse的success标志文件未生成
if [ ! -f $sse_success_flag ];then
	...
	cd $dirpath/../cmd/;sh sse.sh $colony_num "$@"
fi
# szse的success标志文件未生成
...
# sse_late的success标志文件未生成
if [ ! -f $sse_late_success_flag ];then
	echo "拉取上海产品文件成功标志文件未生成, 重新拉取上海产品文件"
	sleep 5
	echo "sh sse_late.sh $colony_num $@"
	cd $dirpath/../cmd/;sh sse_late.sh $colony_num "$@"
fi
# szse_late的success标志文件未生成
if [ ! -f $szse_late_success_flag ];then
	echo "拉取深圳产品文件成功标志文件未生成, 重新拉取深圳产品文件"
	sleep 5
	echo "sh szse_late.sh $colony_num $@"
	cd $dirpath/../cmd/;sh szse_late.sh $colony_num "$@"
fi
# csdc的success标志文件未生成
...
# 检测所有success标志文件都生成
if [ -f $mon_success_flag ]&&[ -f $counter_fetch_success_flag ]&&[ -f $counter_distribute_success_flag ]&&[ -f $sse_success_flag ]&&[ -f $szse_success_flag ]&&[ -f $sse_late_success_flag ]&&[ -f $szse_late_success_flag ]&&[ -f $csdc_success_flag ];then
	echo "所有标志文件都已经生成, 开始重启oes集群服务器"
	sleep 5
	echo "sh oes_restart.sh $colony_num;sh oes_set_status.sh $colony_num RESET 5"
	cd $dirpath;sh oes_restart.sh $colony_num nct "$@";sh oes_set_status.sh $colony_num RESET 5 "$@"
else
	echo "标志文件未完全生成, 请检查"
	exit 1
fi

#!/home/quant/Document/project/py-dango/.venv/bin/python3.12
...
HOST_CONF_DIR = STORAGE_DIR.joinpath("infrastructure", "host")
MON_DIR = STORAGE_DIR.joinpath("mon")
OES_DIR = STORAGE_DIR.joinpath("oes")
APP_DIR = Path(__file__).parents[2].absolute()
SCRIPT_DIR = APP_DIR.joinpath("script")
PLAYBOOK_DIR = APP_DIR.joinpath("playbook")
...
def init_vars(config_path: Path, extravars: str = ""):
    ...
    with open(colony_path, "r") as f:
        vars = json.load(f)
    vars["mon_host"] = load_mon_conf(vars["mon_host_id"])
    if extravars:

...
def main(task: str, mds_dir: Path, colony_num: str, curr_date: Optional[str] = None):
    automatic_path = mds_dir.joinpath("config", colony_num, "all", "automatic.yaml")
    ...
    mon_etf_path = mds_dir.joinpath("mon", colony_num, "EtfTradeList.csv")
    counter_etf_path = mds_dir.joinpath("counter", colony_num, "data", "broker", f"EtfTradeList{str(curr_date)[4:]}.csv")
    if task == "sse":
        tmp_path = mds_dir.joinpath(".tmp", colony_num, "sse_etf.yaml")
        create_sse_var_file(automatic, mon_etf_path, counter_etf_path, tmp_path)
    elif task == "szse":
        tmp_path = mds_dir.joinpath(".tmp", colony_num, "szse_etf.yaml")
        create_szse_var_file(automatic, mon_etf_path, counter_etf_path, tmp_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="本脚本用于执行mds的playbook")
    parser.add_argument(
        "--task", 
        type=str, 
